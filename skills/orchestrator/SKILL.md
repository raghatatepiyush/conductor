...
> 🛑 **One conductor, not two.** If `dimdim-agents` (a peer orchestrator) is installed, don't hand the whole task to `/dimdim` from inside a Conductor run — two orchestrators means two plans and two gates. Keep leading; borrow a *single* DIMDIM specialist sub-agent only when it's the sharpest tool for one lane. See `references/routing-and-plugins.md`.
