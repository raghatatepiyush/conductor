...
| **A peer multi-agent orchestrator** *(if installed)* | `dimdim-agents` *(plugin)* — DIMDIM lead + specialist sub-agents | `/dimdim`, `/design-agent`, `/discover-agent`, `/frontend-agent`, `/sdlc-agent`, `/security-agent`, `/md-agent` | `/plugin install dimdim-agents@claude-plugins-official` | **Conductor *is* the orchestrator — don't stack a second one.** Borrow only an individual specialist sub-agent when it's the sharpest tool (see note below) |

> **Routing `dimdim-agents` (when-to-fire signals — read this; it's an orchestrator, not a leaf tool).** DIMDIM is itself a lead orchestrator with its own sub-agent team and its own safety rails (it also never commits/pushes/deploys without explicit user instruction — same doctrine as Conductor). Two orchestrators driving one task is a recipe for double-planning, double-gating, and confusion. So:
> - **Default: don't invoke `/dimdim` from inside a Conductor run.** You are already conducting; handing the baton to a second conductor means two plans and two approval gates for one task. If the human explicitly wants DIMDIM to lead (e.g. "use DIMDIM for this"), step back and let it — don't run both.
> - **Do borrow a *single specialist sub-agent* when it's genuinely the best fit and you're not already covering that lane** — e.g. `/discover-agent` to map an unfamiliar codebase, `/frontend-agent` for its three-viewport Playwright screenshot pass, `/sdlc-agent` for a deep code lane. Fire signal: the task names DIMDIM, or the project is a DIMDIM/FitApex project (its `agent_*.md` memory files and `/ebm-sorta-agent` are FitApex-specific), or you want one of its sub-agents' specific strengths.
> - **Skip `/ebm-sorta-agent` unless the project is FitApex / health-fitness content** — it enforces Evidence-Based-Medicine review and is domain-specific, not general.
>
> In short: when both are present, **one conductor leads**. Prefer Conductor's own routing; reach into DIMDIM's bench for a specific player, not its whole orchestra.
