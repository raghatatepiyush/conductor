...
**Frame the job — which mode?** Two intents need different plans: *(a) Targeted* — test a specific change, bug, ticket, or component (drive the cases from its behavior and acceptance criteria). *(b) Coverage-backfill* — raise coverage across a module or codebase that already exists but lacks tests (first audit existing coverage **depth** and rank gaps by risk, using the taxonomy in `references/test-design-principles.md`, then attack highest-risk gaps first). State which mode you're in; if a request blends both, say so and sequence them. **Starting from a requirement instead of from code?** If the job is to write brutally thorough test **scenarios** from a Jira/Confluence ticket *before* any test exists, that's the sibling **`scenarios-from-requirements`** skill — route there, then bring its confirmed scenarios back here for the write→run phases.
...
- The sibling **`scenarios-from-requirements`** skill (`skills/scenarios-from-requirements/`) carries the requirements-first playbook — reading a Jira/Confluence source of truth, interrogating it for genuine gaps, and writing fully-traceable **scenarios** (with an HTML/CSV coverage report) *before* any code. **Route there when the job starts from a requirement, not from code; it hands its confirmed scenarios back to this Test Architect for the write→run phases.**
